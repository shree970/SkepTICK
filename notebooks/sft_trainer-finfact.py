...
#   - Run from root of trl repo with: accelerate launch --config_file=examples/accelerate_configs/deepspeed_zero1.yaml examples/scripts/sft_trainer-facts.py
import os
...
class ScriptArguments:
    ...
    seq_length: Optional[int] = field(
        default=1536, metadata={"help": "Input sequence length"}
    )
    ...
    peft_lora_alpha: Optional[int] = field(
        default=64, metadata={"help": "the alpha parameter of the LoRA adapters"}
    )
    logging_steps: Optional[int] = field(
        default=50, metadata={"help": "the number of logging steps"}
    )
    ...
    push_to_hub: Optional[bool] = field(
        default=True, metadata={"help": "Push the model to HF Hub"}
    )
    hub_model_id: Optional[str] = field(
        default="mistral-7b-finetuned-finfacts",
...
train_df = pd.read_csv("../facts/train_df.csv")
val_df = pd.read_csv("../facts/val_df.csv")
...
# Step 7: Merge peft weights to LLM and push to hub
# model_name = "mistralai/Mistral-7B-Instruct-v0.1"
# new_model = "finfact-classifier-noise"
